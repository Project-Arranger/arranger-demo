...
import { selectLaunchpadDrumsClip } from '../src/app/launchpadDrumsClipSelection.js';
...
function createHarness({ canSelectClip = () => true } = {}) {
  const checkpoints = [];
  const seeks = [];
  const select = (bar) => selectLaunchpadDrumsClip({
    bar,
    canSelectClip,
...
test('direct hardware selection creates a missing Drums clip once with undo', () => {
  const { checkpoints, select } = createHarness();

  assert.deepEqual(select(4), { ok: true, created: true, bar: 4 });
  assert.equal(useMusicStore.getState().selectedClipId, 'drums-bar-4');
  assert.equal(useMusicStore.getState().selectedBar, 4);
  assert.deepEqual(useMusicStore.getState().matrix.drums[4], Array(16).fill(null));
  assert.deepEqual(checkpoints, ['checkpoint']);

  assert.deepEqual(select(4), { ok: true, created: false, bar: 4 });
  assert.deepEqual(checkpoints, ['checkpoint']);
  assert.equal(useMusicStore.getState().clips.ids.filter((id) => id === 'drums-bar-4').length, 1);
});

test('hardware selection respects Drums context and tutorial clip guards', () => {
  const blocked = createHarness({ canSelectClip: () => false });
  assert.deepEqual(blocked.select(3), { ok: false, reason: 'blocked' });
  assert.equal(useMusicStore.getState().getClipForTrackBar('drums', 3), null);
  ...
  assert.deepEqual(inactive.select(5), { ok: false, reason: 'inactive-drums' });
  assert.equal(useMusicStore.getState().getClipForTrackBar('drums', 5), null);
});

test('hardware clip selection seeks to the target bar start whether playing or stopped', () => {
  const { seeks, select } = createHarness();
  useMusicStore.getState().createClip('drums', 3);
  useMusicStore.getState().selectClip('drums-bar-0');
  ...
  useMusicStore.getState().selectClip('drums-bar-0');
  useMusicStore.setState({ currentBar: 0, currentStep: 4, isPlaying: true });
  select(3);
  assert.deepEqual(seeks, [[3, 0], [3, 0]]);

  useMusicStore.setState({ currentBar: 3, currentStep: 8, isPlaying: false });
  select(3);
  assert.deepEqual(seeks, [[3, 0], [3, 0], [3, 0]]);
