...
test('tutorial task targets make allowed cells and bars obvious', async () => {
  ...
  assert.match(finalTargetButtonBlock, /--tutorial-highlight-accent:\s*rgb\(255 239 199 \/ 0\.86\);/);
  assert.match(finalTargetButtonBlock, /--tutorial-highlight-rest-filter:\s*brightness\(1\.12\) saturate\(1\.04\);/);
  assert.match(finalTargetButtonBlock, /--tutorial-highlight-peak-filter:\s*brightness\(1\.22\) saturate\(1\.08\);/);
  assert.match(finalTargetButtonBlock, /--tutorial-control-highlight-glow:\s*rgb\(255 239 199 \/ 0\.24\);/);
  assert.match(finalTargetButtonBlock, /--tutorial-control-highlight-peak-glow:\s*rgb\(255 244 214 \/ 0\.34\);/);
  ...
  assert.match(finalTargetButtonBlock, /background:[\s\S]*rgb\(255 252 232 \/ 0\.95\)[\s\S]*rgb\(255 232 178 \/ 0\.72\)[\s\S]*var\(--hardware-control-bg\)/);
  assert.doesNotMatch(finalTargetButtonBlock, /var\(--asset-brass\)/);
  assert.match(finalTargetButtonBlock, /border:\s*1px solid rgb\(255 246 218 \/ 0\.82\);/);
  assert.match(finalTargetButtonBlock, /appearance:\s*none;/);
  assert.match(finalTargetButtonBlock, /color:\s*#2a170d;/);
  assert.match(finalTargetButtonBlock, /text-shadow:\s*0 1px 0 rgb\(255 246 218 \/ 0\.58\);/);
  const finalFillTargetMatch = css.match(/\.track-main-row:has\(\.fill-empty-clips\.tutorial-control-target\)\s*\{(?<rowBlock>[\s\S]*?)\n\}[\s\S]*?\.fill-empty-clips\.tutorial-control-target\s*\{(?<buttonBlock>[\s\S]*?)\n\}/);
  ...
  assert.doesNotMatch(finalFillTargetMatch.groups.buttonBlock, /var\(--c-drums\)|var\(--track-color/);
  assert.match(finalFillTargetMatch.groups.buttonBlock, /--tutorial-highlight-accent:\s*rgb\(255 239 199 \/ 0\.86\);/);
  assert.match(finalFillTargetMatch.groups.buttonBlock, /--tutorial-target-ink:\s*#2a170d;/);
  assert.match(finalFillTargetMatch.groups.buttonBlock, /--tutorial-target-surface:\s*rgb\(255 244 214 \/ 0\.9\);/);
  assert.match(finalFillTargetMatch.groups.buttonBlock, /--tutorial-highlight-rest-shadow:[\s\S]*var\(--tutorial-control-highlight-glow\)/);
  ...
  assert.match(finalFillTargetMatch.groups.buttonBlock, /background:[\s\S]*rgb\(255 252 232 \/ 0\.92\)[\s\S]*var\(--hardware-control-bg\)/);
  assert.match(finalFillTargetMatch.groups.buttonBlock, /border:\s*1px solid rgb\(255 246 218 \/ 0\.78\);/);
  assert.match(finalFillTargetMatch.groups.buttonBlock, /box-shadow:[\s\S]*var\(--tutorial-highlight-rest-shadow\)/);
  ...
  const fillTargetGemBlock = css.match(/\.fill-empty-clips\.tutorial-control-target \.fill-gem\s*\{(?<block>[\s\S]*?)\n\}/)?.groups.block;
  ...
  assert.match(fillTargetGemBlock, /filter:\s*brightness\(1\.18\) saturate\(1\.04\);/);
  assert.match(fillTargetGemBlock, /box-shadow:[\s\S]*0 0 14px rgb\(255 239 199 \/ 0\.32\)/);
  ...
  assert.match(css, /\.btn-template-groove\.tutorial-control-target svg\s*\{[^}]*color:\s*#2a170d;[^}]*filter:\s*drop-shadow\(0 1px 0 rgb\(255 246 218 \/ 0\.58\)\);/s);
