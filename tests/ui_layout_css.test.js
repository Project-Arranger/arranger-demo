...
test('groove template picker mirrors the reference secondary menu layout', async () => {
  ...
  assert.match(css, /\.tpl-card\.tutorial-control-target\s*\{[^}]*--tutorial-highlight-rest-shadow:\s*var\(--tutorial-target-contained-rest-shadow\);/s);
  assert.match(css, /\.tpl-card\.tutorial-control-target\s*\{[^}]*background:\s*color-mix\(in oklab,\s*white 78%,\s*var\(--tutorial-target-surface\)\);/s);
  assert.match(css, /\.tpl-card\.tutorial-control-target\s*\{[^}]*border:\s*1px solid var\(--tutorial-target-surface-border\);/s);
...
test('tutorial task targets make allowed cells and bars obvious', async () => {
  ...
  assert.match(finalFillTargetSurfaceBlock, /--tutorial-highlight-accent:\s*var\(--track-color,\s*var\(--c-drums\)\);/);
  assert.match(finalFillTargetSurfaceBlock, /--tutorial-highlight-ink:\s*var\(--track-ink,\s*var\(--c-drums-ink\)\);/);
  assert.match(finalFillTargetSurfaceBlock, /--tutorial-target-ink:\s*color-mix\(in oklab,\s*var\(--track-ink,\s*var\(--c-drums-ink\)\) 84%,\s*black\);/);
  assert.match(finalFillTargetSurfaceBlock, /--tutorial-target-surface:\s*color-mix\(in oklab,\s*white 92%,\s*var\(--track-color,\s*var\(--c-drums\)\)\);/);
  assert.match(finalFillTargetSurfaceBlock, /--tutorial-target-surface-border:\s*color-mix\(in oklab,\s*var\(--track-color,\s*var\(--c-drums\)\) 72%,\s*white\);/);
  assert.match(finalFillTargetSurfaceBlock, /--tutorial-highlight-rest-filter:\s*brightness\(1\.32\) saturate\(1\.24\);/);
  assert.match(finalFillTargetSurfaceBlock, /--tutorial-highlight-peak-filter:\s*brightness\(1\.72\) saturate\(1\.46\);/);
  assert.match(finalFillTargetSurfaceBlock, /--tutorial-highlight-rest-shadow:\s*var\(--tutorial-target-contained-rest-shadow\);/);
  assert.match(finalFillTargetSurfaceBlock, /--tutorial-highlight-peak-shadow:\s*var\(--tutorial-target-contained-peak-shadow\);/);
  ...
  assert.match(finalFillTargetSurfaceBlock, /background:[\s\S]*rgb\(255 254 244 \/ 0\.98\)[\s\S]*var\(--tutorial-target-surface\);/);
  assert.doesNotMatch(finalFillTargetSurfaceBlock, /var\(--hardware-control-bg\)/);
  assert.match(finalFillTargetSurfaceBlock, /border:\s*1px solid var\(--tutorial-target-surface-border\);/);
  ...
  const fillTargetGemBlocks = [...css.matchAll(/\.fill-empty-clips\.tutorial-control-target \.fill-gem\s*\{(?<block>[\s\S]*?)\n\}/g)]
    .map((match) => match.groups.block);
  const baseFillTargetGemBlock = fillTargetGemBlocks.find((block) => /width:\s*30px;/.test(block));
  const fillTargetGemBlock = fillTargetGemBlocks.filter((block) => block.includes('brightness(1.34)')).at(-1);
  assert.ok(fillTargetGemBlock, 'tutorial fill target should brighten the gem itself');
  assert.ok(baseFillTargetGemBlock, 'tutorial fill target should keep the base gem enlarged');
  assert.match(baseFillTargetGemBlock, /width:\s*30px;/);
  assert.match(baseFillTargetGemBlock, /height:\s*30px;/);
  assert.match(fillTargetGemBlock, /border-color:\s*color-mix\(in oklab,\s*var\(--track-ink,\s*var\(--c-drums-ink\)\) 42%,\s*#f7de9d\);/);
  assert.match(fillTargetGemBlock, /filter:\s*brightness\(1\.34\) saturate\(1\.44\);/);
  assert.match(fillTargetGemBlock, /box-shadow:[\s\S]*0 0 16px color-mix\(in oklab,\s*var\(--track-color,\s*var\(--c-drums\)\) 64%,\s*transparent\)/);
