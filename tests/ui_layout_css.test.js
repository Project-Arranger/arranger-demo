...
test('tutorial task targets make allowed cells and bars obvious', async () => {
  ...
  assert.match(css, /\.track:has\(\.fill-empty-clips\.tutorial-control-target\)\s*\{[^}]*position:\s*relative;[^}]*z-index:\s*calc\(var\(--tutorial-target-z\) \+ 1\);/s);
  assert.match(css, /\.track:has\(\.fill-empty-clips\.tutorial-control-target\) \.fill-empty-clips\.tutorial-control-target\s*\{[^}]*z-index:\s*calc\(var\(--tutorial-target-z\) \+ 2\);/s);
  assert.match(finalFillTargetMatch.groups.rowBlock, /grid-template-columns:\s*minmax\(0,\s*1fr\) minmax\(72px,\s*80px\);/);
  assert.doesNotMatch(finalFillTargetMatch.groups.buttonBlock, /var\(--c-drums\)|var\(--track-color|var\(--tutorial-target-contained-/);
  assert.match(finalFillTargetMatch.groups.buttonBlock, /--tutorial-highlight-accent:\s*rgb\(255 230 205 \/ 0\.82\);/);
  assert.match(finalFillTargetMatch.groups.buttonBlock, /--tutorial-target-ink:\s*#2a170d;/);
  assert.match(finalFillTargetMatch.groups.buttonBlock, /--tutorial-target-surface:\s*rgb\(255 244 224 \/ 0\.9\);/);
  assert.match(finalFillTargetMatch.groups.buttonBlock, /--tutorial-highlight-rest-filter:\s*brightness\(1\.1\) saturate\(0\.94\);/);
  assert.match(finalFillTargetMatch.groups.buttonBlock, /--tutorial-highlight-peak-filter:\s*brightness\(1\.18\) saturate\(0\.98\);/);
  ...
  assert.match(finalFillTargetMatch.groups.buttonBlock, /--tutorial-highlight-peak-shadow:[\s\S]*var\(--tutorial-control-highlight-peak-glow\)/);
  ...
  assert.match(finalFillTargetMatch.groups.buttonBlock, /background:[\s\S]*rgb\(255 253 240 \/ 0\.92\)[\s\S]*var\(--hardware-control-bg\)/);
  assert.match(finalFillTargetMatch.groups.buttonBlock, /border:\s*1px solid rgb\(255 238 214 \/ 0\.78\);/);
  ...
  assert.match(fillTargetGemBlock, /filter:\s*brightness\(1\.16\) saturate\(0\.98\);/);
  assert.match(fillTargetGemBlock, /box-shadow:[\s\S]*0 0 14px rgb\(255 226 198 \/ 0\.3\)/);
