...
test('tutorial sidebar is embedded as a workbench column and reopens from the topbar', async () => {
  ...
  assert.match(css, /\.app-main:has\(\.tutorial-target-active\)::before\s*\{[^}]*position:\s*absolute;/s);
  assert.match(css, /\.app-main:has\(\.tutorial-target-active\)::before\s*\{[^}]*background:\s*color-mix\(in oklab,\s*black 34%,\s*transparent\);/s);
  assert.match(css, /\.app-main:has\(\.tutorial-target-active\)::before\s*\{[^}]*z-index:\s*50;/s);
