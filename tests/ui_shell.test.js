...
test('app shell exposes the chord rhythm editor and unified template workspace', async () => {
  ...
  assert.match(chordEditorSource, /aria-label=\{`切换第 \$\{step \+ 1\} 步`\}/);
  assert.match(chordEditorSource, /onChordRhythmStepToggle\(step\)/);
  ...
  assert.match(chordEditorSource, /progressionTemplateId: pendingTemplateId/);
  assert.match(chordEditorSource, /grooveTemplateId: pendingGrooveTemplateId/);
  assert.match(chordEditorSource, /handleApply\('bar'\)/);
  assert.match(chordEditorSource, /handleApply\('global'\)/);
  ...
  assert.doesNotMatch(chordEditorSource, /丰富和弦|添加经过和弦|清空整轨|More/);
  ...
  assert.match(source, /applyChordTemplateWorkspaceToBar/);
...
test('app routes drums tutorial tasks through guards and target props', async () => {
  ...
  assert.match(chordEditorSource, /chord-template-card:\$\{template\.id\}/);
  assert.match(chordEditorSource, /chord-groove-card:\$\{template\.id\}/);
  assert.match(chordEditorSource, /chord-template-apply-current/);
  assert.match(chordEditorSource, /chord-template-apply-global/);
  assert.match(chordEditorSource, /tutorial-control-target/);
  assert.match(source, /handleChordTemplateWorkspaceApply = useCallback\([\s\S]*handleTutorialControlAction\(\{[\s\S]*CHORD_TEMPLATE_APPLY_GLOBAL[\s\S]*CHORD_TEMPLATE_APPLY_CURRENT/);
  assert.match(tutorialRuntimeSource, /CHORD_SELECT_PROGRESSION_TEMPLATE[\s\S]*CHORD_TEMPLATE_WORKSPACE_BUTTON[\s\S]*CHORD_TEMPLATE_APPLY_GLOBAL/);
